...
namespace :i18n do
  ...
  task modularize: :environment do
    ...
    modules = {
      ...
      'forms' => %w[form_ field_ submit submitter template_ document_ upload draw signature initials checkbox radio cells stamp payment phone],
      ...
      'common' => %w[language_ yes no ok cancel save saving update updating delete deleting remove removing 
    ...
    translations.each do |locale, keys|
      next if %w[en-US en-GB es-ES fr-FR pt-PT de-DE it-IT nl-NL].include?(locale.to_s)
